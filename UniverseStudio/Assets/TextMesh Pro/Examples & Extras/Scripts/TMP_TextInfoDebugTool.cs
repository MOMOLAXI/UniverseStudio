    ...
    public class TMP_TextInfoDebugTool : MonoBehaviour
    {
        ...
        void DrawCharactersBounds()
        {
            ...
            for (int i = 0; i < characterCount; i++)
            {
                ...
                Vector3 topLeft = m_Transform.TransformPoint(new(characterInfo.topLeft.x, characterInfo.topLeft.y, 0));
                Vector3 topRight = m_Transform.TransformPoint(characterInfo.topRight);
                Vector3 bottomRight = m_Transform.TransformPoint(new(characterInfo.bottomRight.x, characterInfo.bottomRight.y, 0));
                ...
                else
                {
                    ...
                    DrawDottedRectangle(m_Transform.TransformPoint(new(characterInfo.origin, characterInfo.descender, 0)), m_Transform.TransformPoint(new(whiteSpaceAdvance, characterInfo.ascender, 0)), color, 4);
                }
                ...
                //Draw Ascent line
                Vector3 ascentlineStart = m_Transform.TransformPoint(new(origin, ascentline, 0));
                Vector3 ascentlineEnd = m_Transform.TransformPoint(new(advance, ascentline, 0));
                ...
                Vector3 capHeightStart = new(topLeft.x, m_Transform.TransformPoint(new(0, capline, 0)).y, 0);
                Vector3 capHeightEnd = new(topRight.x, m_Transform.TransformPoint(new(0, capline, 0)).y, 0);

                float meanline = characterInfo.fontAsset == null ? 0 : baseline + characterInfo.fontAsset.faceInfo.meanLine * characterInfo.scale;
                Vector3 meanlineStart = new(topLeft.x, m_Transform.TransformPoint(new(0, meanline, 0)).y, 0);
                Vector3 meanlineEnd = new(topRight.x, m_Transform.TransformPoint(new(0, meanline, 0)).y, 0);
                ...
                //Draw Base line
                Vector3 baselineStart = m_Transform.TransformPoint(new(origin, baseline, 0));
                Vector3 baselineEnd = m_Transform.TransformPoint(new(advance, baseline, 0));

                Handles.color = Color.cyan;
                Handles.DrawDottedLine(baselineStart, baselineEnd, dottedLineSize);

                //Draw Descent line
                Vector3 descentlineStart = m_Transform.TransformPoint(new(origin, descentline, 0));
                Vector3 descentlineEnd = m_Transform.TransformPoint(new(advance, descentline, 0));

                Handles.color = Color.cyan;
                Handles.DrawDottedLine(descentlineStart, descentlineEnd, dottedLineSize);

                // Draw Origin
                Vector3 originPosition = m_Transform.TransformPoint(new(origin, baseline, 0));
                DrawCrosshair(originPosition, 0.05f / m_ScaleMultiplier, Color.cyan);

                // Draw Horizontal Advance
                Vector3 advancePosition = m_Transform.TransformPoint(new(advance, baseline, 0));
               ...
               if (m_HandleSize < 0.5f)
               {
                   GUIStyle style = new(GUI.skin.GetStyle("Label"))
                   {
                       normal =
                       {
                           textColor = new(0.6f, 0.6f, 0.6f, 1.0f)
                       },
                       fontSize = 12,
                       fixedWidth = 200,
                       fixedHeight = 20
                   ...
                   // Ascent Line
                   labelPosition = m_Transform.TransformPoint(new(center, ascentline, 0));
                   ...
                   // Base Line
                   labelPosition = m_Transform.TransformPoint(new(center, baseline, 0));
                   Handles.Label(labelPosition, "Base Line", style);
                   //Handles.Label(labelPosition, "Base Line (" + baselineMetrics.ToString("f3") + ")" , style);

                   // Descent line
                   labelPosition = m_Transform.TransformPoint(new(center, descentline, 0));
                ...
                for (int j = 0; j < wInfo.characterCount; j++)
                {
                    ...
                    if (isBeginRegion == false && isCharacterVisible)
                    {
                        ...
                        if (wInfo.characterCount == 1)
                        {
                            ...
                            topLeft = m_Transform.TransformPoint(new(topLeft.x, maxAscender, 0));
                            bottomLeft = m_Transform.TransformPoint(new(bottomLeft.x, minDescender, 0));
                            bottomRight = m_Transform.TransformPoint(new(currentCharInfo.topRight.x, minDescender, 0));
                            topRight = m_Transform.TransformPoint(new(currentCharInfo.topRight.x, maxAscender, 0));
        ...
        void DrawLineBounds()
        {
            ...
            for (int i = 0; i < lineCount; i++)
            {
                ...
                // Draw Ascent line
                Vector3 ascentlineStart = m_Transform.TransformPoint(new(lineBottomLeft, ascentline, 0));
                Vector3 ascentlineEnd = m_Transform.TransformPoint(new(lineTopRight, ascentline, 0));

                Handles.color = Color.yellow;
                Handles.DrawDottedLine(ascentlineStart, ascentlineEnd, dottedLineSize);

                // Draw Base line
                Vector3 baseLineStart = m_Transform.TransformPoint(new(lineBottomLeft, baseline, 0));
                Vector3 baseLineEnd = m_Transform.TransformPoint(new(lineTopRight, baseline, 0));

                Handles.color = Color.yellow;
                Handles.DrawDottedLine(baseLineStart, baseLineEnd, dottedLineSize);

                // Draw Descent line
                Vector3 descentLineStart = m_Transform.TransformPoint(new(lineBottomLeft, descentline, 0));
                Vector3 descentLineEnd = m_Transform.TransformPoint(new(lineTopRight, descentline, 0));
                ...
                if (m_HandleSize < 1.0f)
                {
                    GUIStyle style = new()
                    {
                        normal =
                        {
                            textColor = new(0.8f, 0.8f, 0.8f, 1.0f)
                        },
                        fontSize = 12,
                        fixedWidth = 200,
                        fixedHeight = 20
                    ...
                    // Ascent Line
                    labelPosition = m_Transform.TransformPoint(new(lineBottomLeft, ascentline, 0));
                    style.padding = new(0, 10, 0, 5);
                    style.alignment = TextAnchor.MiddleRight;
                    Handles.Label(labelPosition, "Ascent Line", style);

                    // Base Line
                    labelPosition = m_Transform.TransformPoint(new(lineBottomLeft, baseline, 0));
                    Handles.Label(labelPosition, "Base Line", style);

                    // Descent line
                    labelPosition = m_Transform.TransformPoint(new(lineBottomLeft, descentline, 0));
                    Handles.Label(labelPosition, "Descent Line", style);
                }
            }
        }
        ...
        void DrawBounds()
        {
            ...
            DrawRectangle(bottomLeft, topRight, new(1, 0.5f, 0));
        }
        ...
        void DrawTextBounds()
        {
            ...
            DrawRectangle(bottomLeft, topRight, new(0f, 0.5f, 0.5f));
        }
        ...
        {
            Handles.color = color;
            Handles.DrawDottedLine(bottomLeft, new(bottomLeft.x, topRight.y, bottomLeft.z), size);
            Handles.DrawDottedLine(new(bottomLeft.x, topRight.y, bottomLeft.z), topRight, size);
            Handles.DrawDottedLine(topRight, new(topRight.x, bottomLeft.y, bottomLeft.z), size);
            Handles.DrawDottedLine(new(topRight.x, bottomLeft.y, bottomLeft.z), bottomLeft, size);
        }
        ...
        void DrawCrosshair(Vector3 position, float size, Color color)
        {
            ...
            Handles.DrawLine(new(position.x - size, position.y, position.z), new(position.x + size, position.y, position.z));
            Handles.DrawLine(new(position.x, position.y - size, position.z), new(position.x, position.y + size, position.z));
        }
