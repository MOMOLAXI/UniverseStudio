...
namespace Michsky.MUIP
{
    ...
    public class UIGradient : BaseMeshEffect
    {
        ...
        [SerializeField]
        UnityEngine.Gradient _effectGradient = new() { colorKeys = new GradientColorKey[] { new(Color.black, 0), new(Color.white, 1) } };
        ...
        Rect GetBounds(List<UIVertex> vertices)
        {
            ...
            return new(left, bottom, right - left, top - bottom);
        }
        ...
        UIVertex CreateSplitVertex(UIVertex vertex1, UIVertex vertex2, float stop)
        {
            if (GradientType == Type.Horizontal)
            {
                ...
                {
                    position = new(stop, splitY, vertex1.position.z),
            ...
            else
            {
                ...
                UIVertex splitVertex = new()
                {
                    position = new(splitX, stop, vertex1.position.z),
                    normal = vertex1.normal,
                    uv0 = new Vector2(vertex1.uv0.x - (uvx * ratio), vertex1.uv0.y - (uvy * ratio)),
                    color = Color.white
                };
