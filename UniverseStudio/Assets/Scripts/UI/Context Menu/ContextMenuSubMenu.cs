       ...
       [HideInInspector] public bool enableFadeOut = true;
        ...
        public void OnPointerEnter(PointerEventData eventData)
        {
            ...
            for (int i = 0; i < cmContent.contexItems[subMenuIndex].subMenuItems.Count; ++i)
            {
                ...
                {
                    GameObject go = Instantiate(selectedItem, new(0, 0, 0), Quaternion.identity) as GameObject;
                    ...
                    if (cmContent.contexItems[subMenuIndex].subMenuItems[i].contextItemType == ContextMenuContent.ContextItemType.Button)
                    {
                        ...
                        if (imageHelper == null)
                            setItemImage.color = new(0, 0, 0, 0);
            ...
            {
                if (cmManager.bottomLeft == true) { listParent.pivot = new(0f, listParent.pivot.y); }
                if (cmManager.bottomRight == true) { listParent.pivot = new(1f, listParent.pivot.y); }
                if (cmManager.topLeft == true) { listParent.pivot = new(listParent.pivot.x, 0f); }
                if (cmManager.topRight == true) { listParent.pivot = new(listParent.pivot.x, 1f); }
            }
