...
namespace Michsky.MUIP
{
    ...
    public class ContextMenuManager : MonoBehaviour
    {
        ...
        {
            if (uiPos.x <= -100) { contentPos = new(hBorderLeft, contentPos.y, 0); contentRect.pivot = new(0f, contentRect.pivot.y); bottomLeft = true; }
            else { bottomLeft = false; }

            if (uiPos.x >= 100) { contentPos = new(hBorderRight, contentPos.y, 0); contentRect.pivot = new(1f, contentRect.pivot.y); bottomRight = true; }
            else { bottomRight = false; }

            if (uiPos.y <= -75) { contentPos = new(contentPos.x, vBorderBottom, 0); contentRect.pivot = new(contentRect.pivot.x, 0f); topLeft = true; }
            else { topLeft = false; }

            if (uiPos.y >= 75) { contentPos = new(contentPos.x, vBorderTop, 0); contentRect.pivot = new(contentRect.pivot.x, 1f); topRight = true; }
            else { topRight = false; }
        }
        ...
        public void SetContextMenuPosition()
        {
            ...
            if (mainCanvas.renderMode == RenderMode.ScreenSpaceCamera || mainCanvas.renderMode == RenderMode.WorldSpace)
            {
                ...
                contextRect.localPosition = new(contextRect.localPosition.x, contextRect.localPosition.y, 0);
            ...
            else if (mainCanvas.renderMode == RenderMode.ScreenSpaceOverlay)
            {
                ...
                contextContent.transform.position = new(cursorPos.x + contentPos.x, cursorPos.y + contentPos.y, 0);
            }
        }
