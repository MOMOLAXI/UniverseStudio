...
namespace Michsky.MUIP
{
    public class TooltipManager : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            sourceRect.anchorMin = new(0, 0);
            sourceRect.anchorMax = new(1, 1);
            sourceRect.offsetMin = new(0, 0);
            sourceRect.offsetMax = new(0, 0);

            tooltipContent.GetComponent<RectTransform>().pivot = new(0f, tooltipContent.GetComponent<RectTransform>().pivot.y);
            tooltipContent.GetComponent<RectTransform>().pivot = new(tooltipContent.GetComponent<RectTransform>().pivot.x, 0f);
            ...
            contentPos = new(vBorderTop, hBorderLeft, 0);
        ...
        void CheckForPosition()
        {
            ...
            if (mainCanvas.renderMode == RenderMode.ScreenSpaceCamera || mainCanvas.renderMode == RenderMode.WorldSpace)
            {
                ...
                tooltipRect.localPosition = new(tooltipRect.localPosition.x, tooltipRect.localPosition.y, 0);
        ...
        void CheckForBounds()
        {
            if (uiPos.x <= xLeft)
            {
                contentPos = new(hBorderLeft, contentPos.y, 0);
                contentRect.pivot = new(0f, contentRect.pivot.y);
            }

            else if (uiPos.x >= xRight)
            {
                contentPos = new(hBorderRight, contentPos.y, 0);
                contentRect.pivot = new(1f, contentRect.pivot.y);
            }

            if (uiPos.y <= yTop)
            {
                contentPos = new(contentPos.x, vBorderBottom, 0);
                contentRect.pivot = new(contentRect.pivot.x, 0f);
            }

            else if (uiPos.y >= yBottom)
            {
                contentPos = new(contentPos.x, vBorderTop, 0);
                contentRect.pivot = new(contentRect.pivot.x, 1f);
            }
        }
    }
}