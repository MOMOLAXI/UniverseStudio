    ...
    public class UIManagerSlider : MonoBehaviour
    {
        ...
        void UpdateSlider()
        {
            if (UIManagerAsset.sliderThemeType == UIManager.SliderThemeType.Basic)
            {
                ...
                if (hasLabel == true)
                {
                    if (overrideColors == false) { label.color = new(UIManagerAsset.sliderColor.r, UIManagerAsset.sliderColor.g, UIManagerAsset.sliderColor.b, label.color.a); }
                    if (overrideFonts == false) { label.font = UIManagerAsset.sliderLabelFont; }
                }

                if (hasPopupLabel == true)
                {
                    if (overrideColors == false) { popupLabel.color = new(UIManagerAsset.sliderPopupLabelColor.r, UIManagerAsset.sliderPopupLabelColor.g, UIManagerAsset.sliderPopupLabelColor.b, popupLabel.color.a); }
            ...
            else if (UIManagerAsset.sliderThemeType == UIManager.SliderThemeType.Custom)
            {
                ...
                if (hasLabel == true)
                {
                    if (overrideColors == false) { label.color = new(UIManagerAsset.sliderLabelColor.r, UIManagerAsset.sliderLabelColor.g, UIManagerAsset.sliderLabelColor.b, label.color.a); }
