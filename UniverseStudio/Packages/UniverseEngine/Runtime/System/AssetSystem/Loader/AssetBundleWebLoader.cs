...
namespace Universe
{
	sealed internal class AssetBundleWebLoader : BundleLoaderBase
	{
		private enum ESteps
		{
			None = 0,
		...
		public AssetBundleWebLoader(AssetPackageProxy proxy, BundleInfo bundleInfo) : base(proxy, bundleInfo)
		{
		}
...
#endif
				{
					Log.Warning($"Failed to get asset bundle from web : {FileLoadPath} Error : {_webRequest.error}");
		...
		public override void WaitForAsyncComplete()
		{
			if (_isShowWaitForAsyncError == false)
			{
				...
				Log.Error($"WebGL platform not support {nameof(WaitForAsyncComplete)} ! Use the async load method instead of the sync load method !");
			}
		}
	}
}